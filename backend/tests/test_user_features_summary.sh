...
# First do a search to get a search ID
SEARCH_RESPONSE=$(curl -s -X POST \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $USER_TOKEN" \
...
SEARCH_ID=$(echo "$SEARCH_RESPONSE" | jq -r '.search_id // empty')

if [ -n "$SEARCH_ID" ]; then
    # Test CSV export (this will stream a CSV file)
    EXPORT_RESPONSE=$(curl -s -I -X POST \
        -H "Content-Type: application/json" \
        -H "Authorization: Bearer $USER_TOKEN" \
        -d "{\"search_id\":\"$SEARCH_ID\"}" \
        "$BASE_URL/search/export")
    
    # Check if we get CSV headers (Content-Type: text/csv)
    if echo "$EXPORT_RESPONSE" | grep -q "text/csv"; then
        print_status $GREEN "   ✅ Export endpoint working - streams CSV directly"
        FILENAME=$(echo "$EXPORT_RESPONSE" | grep -i "content-disposition" | sed 's/.*filename=\([^;]*\).*/\1/')
        echo "   📄 Filename: $FILENAME"
    else
        print_status $YELLOW "   ⚠️  Export endpoint response differs from expected"
    fi
else
    print_status $YELLOW "   ⚠️  No search ID available for export test"
fi
